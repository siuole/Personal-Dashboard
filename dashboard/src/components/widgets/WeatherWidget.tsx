...
function Stat({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
      {icon}
      <span style={{ fontSize: 11, fontWeight: 500, color: '#6B7280', letterSpacing: 0.1 }}>{label}</span>
    </div>
  );
}

function WindSvg() {
  return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2" strokeLinecap="round"><path d="M9.59 4.59A2 2 0 1 1 11 8H2M12.59 19.41A2 2 0 1 0 14 16H2M17.73 7.73A2.5 2.5 0 1 1 19.5 12H2"/></svg>;
}
function HumiditySvg() {
  return <svg width="11" height="13" viewBox="0 0 24 24" fill="#9CA3AF" stroke="none"><path d="M12 2C12 2 4 11 4 16a8 8 0 0 0 16 0C20 11 12 2 12 2Z"/></svg>;
}
function TempSvg() {
  return <svg width="11" height="13" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2" strokeLinecap="round"><path d="M14 14.76V3.5a2.5 2.5 0 0 0-5 0v11.26a4.5 4.5 0 1 0 5 0Z"/></svg>;
}
  ...
  return (
    <div style={{
      ...
      {forecast.map((day, i) => {
        ...
        return (
          <div key={day.date} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, borderRight: i < forecast.length - 1 ? '1px solid rgba(0,0,0,0.06)' : 'none' }}>
            <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: 0.4, color: isToday ? '#374151' : '#9CA3AF' }}>{name}</span>
            <SmallIcon cat={cat} />
            <span style={{ fontSize: 12, fontWeight: 700, color: '#1F2937' }}>{day.tempMax}°</span>
            <span style={{ fontSize: 11, color: '#C4C9D4' }}>{day.tempMin}°</span>
          </div>
        );
      })}
    </div>
...
export default function WeatherWidget() {
  ...
  return (
    <div style={{ height: '100%', borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{ flex: '1 1 0', padding: '20px 20px 14px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden', background: tint }}>
        <div style={{ position: 'absolute', top: -30, right: -30, width: 130, height: 130, borderRadius: '50%', background: 'rgba(255,255,255,0.35)', pointerEvents: 'none' }}/>

        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 58, fontWeight: 700, color: '#1A1D23', lineHeight: 1, letterSpacing: -3 }}>
              {weather.temp}°
            </div>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#374151', marginTop: 5, letterSpacing: 0.1 }}>
              {weather.cityName}
            </div>
